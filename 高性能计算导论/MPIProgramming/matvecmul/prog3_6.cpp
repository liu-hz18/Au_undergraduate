 ...
 * Run: mpiexec -n <comm_sz> ./main <n> <thread_num>
 */
using namespace std;
/*
...
//#   pragma omp parallel for num_threads(thread_num)
    ...
    //} else {
    //    MPI_Scatterv(NULL, NULL, NULL, submat_mpi_t, loc_A, loc_m * loc_n, MPI_DOUBLE, 0, comm);
    //}
    if( my_rank == 0 ){//使用数据预处理，充分并行
        if(comm_sz > 1){
            block_matrix = new double[n * n];
#           pragma omp parallel for schedule(dynamic)
            for(int i = 0; i < n; i++){
                int offset1 = i/loc_m*col_comm_sz, offset2 = (i%loc_m)*loc_n;
                for(int j = 0; j < n; j++){
                    block_matrix[(offset1+j/loc_n)*local_size + offset2 + j%loc_n] = matrix[i * n + j];
                }
            }
            MPI_Scatter(block_matrix, local_size, MPI_DOUBLE, loc_A, local_size, MPI_DOUBLE, 0, comm);
        } else {
            MPI_Scatter(matrix, local_size, MPI_DOUBLE, loc_A, local_size, MPI_DOUBLE, 0, comm);
        }
    } else {
        MPI_Scatter(NULL, local_size, MPI_DOUBLE, loc_A, local_size, MPI_DOUBLE, 0, comm);
    }
...
#       pragma omp critical
    ...
    *  store the result in the diagonal */
    ...
    if(my_diag_rank == 0){
        ...
        //calculate acc
        double beg_nor = MPI_Wtime();
        double dis = distance(n, parallel_res, serial_res);
        double tot_nor = MPI_Wtime() - beg_nor;
        printf("error(2 norm):                                 %.20lf\n", dis);
        printf("time (Serial):                                 %.6lfs\n", ser_tot);
        printf("time (Total: multiply+communication):          %.6lfs\n", tot[0]);
        printf("time (Matrix-vector Multiply):                 %.6lfs\n", tot[1]);
        printf("time (Scatter&Reduce):                         %.6lfs\n", tot[0] - tot[1]);
        printf("time (Calculate 2-norm):                       %.6lfs\n", tot_nor);
        printf("Speedup ratio(with MPI_Scatter/MPI_Reduce):    %.4lf\n", ser_tot / tot[0]);
        printf("Speedup ratio(without MPI_Scatter/MPI_Reduce): %.4lf\n", ser_tot / tot[1]);
        printf("Efficiency(with MPI_Scatter/MPI_Reduce):       %.4lf\n", (ser_tot / tot[0]) / comm_sz);
        printf("Efficiency(without MPI_Scatter/MPI_Reduce):    %.4lf\n", (ser_tot / tot[1]) / comm_sz / thread_num);
        delete[] serial_res;
        delete[] parallel_res;
        delete[] matrix;
        delete[] vector;
    ...
    if (my_rank == 0 && comm_sz > 1) delete[] block_matrix;
    MPI_Finalize();
}   
