    ...
    <div id="message-board">
        <el-container style="height:100%; border: 1px solid #eee">
            <el-header style="text-align: right; font-size: 10px">
                <el-button style="display: inline-block;margin-right: 15px;" v-on:click="clickPost">
                    <i class="el-icon-edit">发表</i>
                </el-button>
                <!--请修改这两行注释中间的代码完成"刷新"功能-->
                <el-button style="display: inline-block;margin-right: 15px;" v-on:click="onUpdate">
                <!--请修改这两行注释中间的代码完成"刷新"功能-->
        ...
        <PostDialog v-bind:dialogVisible="postDialog.dialogVisible" @add-message-child="addMessage" @close-dialog="closeDialog">
        </PostDialog>
        <!--请补全这两行注释中间的PostDialog-->
        <el-dialog
...
<script>
    ...
    import {getAllMessage, sendNewMessage} from '@/utils/communication.js';
    export default {
        name: "MessageBoard",
        components: {
            MessageList,
        ...
        data(){
            return {
                ...
                messageList: getAllMessage().reverse()
            }
        },
        methods:{
            addMessage: function(title, content, user) {
                var suc = sendNewMessage(title, content, user)
