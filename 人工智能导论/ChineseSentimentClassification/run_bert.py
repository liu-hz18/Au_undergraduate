...
def preprocess():
    ...
    np.save('train_tensor', train_text.reshape(train_text.shape[0], -1))
    np.save('train_lable', train_lable)
    np.save('valid_tensor', valid_text.reshape(valid_text.shape[0], -1))
    np.save('valid_lable', valid_lable)
    np.save('test_tensor', test_text.reshape(test_text.shape[0], -1))
    np.save('test_lable', test_lable)
    print(test_text)
...
# max_length = 64: 0.569
# max_length = 100: 0.570
# max_length = 128: 0.584
# max_length = 150: 0.587
# max_length = 200: 0.589