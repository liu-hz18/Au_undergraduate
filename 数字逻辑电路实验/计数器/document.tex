...
\paragraph*{}
我将两个部分功能实现了集成，在暂停时，可以手动按下clk实现计数器的加一。再次按下pause后，可以开启秒表模式，在当前计数基础上进行每1s一次的计数。在实现过程中，我使用了大量“元件例化”的方法，并且没有使用"+"。本部分代码位于./counter下
...
\begin{lstlisting}[language={VHDL}, title={.vhd}]
